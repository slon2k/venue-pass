...
public sealed class AttendanceOutboxDispatcherTests
{
    private readonly EventsIntegrationTestFixture _fixture;

    public AttendanceOutboxDispatcherTests(EventsIntegrationTestFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        var now = new DateTimeOffset(2026, 6, 27, 12, 5, 0, TimeSpan.Zero);
        var timeProvider = new FakeTimeProvider(now);
    ...
    {
        var now = new DateTimeOffset(2026, 6, 27, 12, 10, 0, TimeSpan.Zero);
        var timeProvider = new FakeTimeProvider(now);
        var services = CreateServices(configureServices: s =>
    ...
    {
        var now = new DateTimeOffset(2026, 6, 27, 12, 30, 0, TimeSpan.Zero);
        var timeProvider = new FakeTimeProvider(now);
        var services = CreateServices(configureServices: s =>
            s.AddScoped<IIntegrationEventHandler<TestIntegrationEvent>, ThrowingHandler>());
    ...
    {
        var now = new DateTimeOffset(2026, 6, 27, 12, 35, 0, TimeSpan.Zero);
        var timeProvider = new FakeTimeProvider(now);
        var services = CreateServices();
    ...
    {
        var now = new DateTimeOffset(2026, 6, 27, 12, 40, 0, TimeSpan.Zero);
        var timeProvider = new FakeTimeProvider(now);
        var services = CreateServices();
    ...
    private string BuildIsolatedConnectionString()
    {
        ...
        {
            InitialCatalog = $"attendance_outbox_test_{Guid.NewGuid():N}"
        };
        return builder.ConnectionString;
    }

    private static AttendanceOutboxDispatcher CreateDispatcher(IServiceProvider services, TimeProvider timeProvider)
        => new(services, timeProvider, NullLogger<AttendanceOutboxDispatcher>.Instance);
