...
| `Tickets` | `IReadOnlyList<Ticket>` | Owned collection, issued on creation |
...
| `OrderItemId` | `OrderItemId` | Which order item produced this ticket |
| `TicketCode` | `string` | Opaque, unique, non-sequential; generated by Ticketing |
| `InventorySeatId` | `InventorySeatId?` | Set for seat tickets |
| `GeneralAdmissionPoolId` | `GeneralAdmissionPoolId?` | Set for GA tickets |
...
Check-in / use lifecycle is deferred to Milestone 05.

### Ticket Issuance Rules

- One `Seat` order item → one `Ticket` with `InventorySeatId` set and `GeneralAdmissionPoolId` null.
- One `GeneralAdmissionPool` order item → `Quantity` tickets, each with `GeneralAdmissionPoolId` set and `InventorySeatId` null.
- Ticket code is opaque, unique, and non-sequential (GUID or CSPRNG-based; implementation detail).
...
| `tickets` | `id`, `order_id`, `order_item_id`, `ticket_code`, `inventory_seat_id`, `general_admission_pool_id`, `status` |
...
| `Domain/Ticket.cs` | `Ticket` entity, `TicketStatus` enum |

#### Infrastructure / Persistence

| File | Purpose |
|------|---------|
| `Infrastructure/Persistence/OrderConfiguration.cs` | EF Core entity type configuration for `Order` and `OrderItem` |
| `Infrastructure/Persistence/TicketConfiguration.cs` | EF Core entity type configuration for `Ticket`; unique index on `ticket_code` |
| EF Core migration | `orders`, `order_items`, `tickets` tables; `UNIQUE(orders.reservation_id)`; `UNIQUE(tickets.ticket_code)` |
...
| File | Change |
|------|--------|
| `ModuleConfiguration.cs` | Register `CheckoutReservationHandler`, `GetOrderHandler`; register `Order` and `Ticket` EF configurations |
...
- [ ] C1: Implement `Order` aggregate, `OrderItem` owned entity, `Ticket` entity, and status enums
- [ ] C2: Add EF Core configuration and migration for `orders`, `order_items`, `tickets` with uniqueness constraints
- [ ] C3: Implement `CheckoutReservationHandler` with inventory sell mutation, ticket issuance, and idempotency path
- [ ] C4: Deliver `POST /reservations/{id}/checkout` endpoint
- [ ] C5: Deliver `GET /orders/{id}` endpoint and handler
- [ ] C6: Unit tests for `Order` creation and ticket issuance rules; integration tests for E4 checkout scenarios
